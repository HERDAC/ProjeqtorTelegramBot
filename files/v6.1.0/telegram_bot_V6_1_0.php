...
if (
	...
	$_SERVER['PHP_AUTH_USER'] !== "AUTH_USER" or
	$_SERVER['PHP_AUTH_PW'] !== "AUTH_PWD"
) {
	error_log("<[Telegram Bot]> HACKING ATTEMPT: ".json_encode($_SERVER));
	exit();
}

// Necessary translations -> files/tg_bot_nls

define("TG_LOG_MSG", 0);
define("TG_LOG_DEBUG", 1);
define("TG_LOG_FULL_DEBUG", 2);

$DEBUG_LEVEL = TG_LOG_MSG;
...
$createOptions = array(
	array(
		array("txt"=>mb_ucfirst(i18nDec("colName")), "name"=>"name", "action"=>"field", "field"=>"name")
	),
	array(
		array("txt"=>mb_ucfirst(i18nDec("colDescription")), "name"=>"description", "action"=>"field", "field"=>"desc")
	),
	array(
		array("txt"=>mb_ucfirst(i18nDec("colIdReferencable")), "name"=>"refElement", "action"=>"field", "field"=>"refElmt")
	),
	array(
		array("txt"=>mb_ucfirst(i18nDec("colModel")), "name"=>"model", "action"=>"field", "field"=>"model"),
		array("txt"=>mb_ucfirst(i18nDec("colResponsible")), "name"=>"responsible", "action"=>"field", "field"=>"resp")
	),
	array(
		array("txt"=>mb_ucfirst(i18nDec("colType")), "name"=>"type", "action"=>"field", "field"=>"type"),
		array("txt"=>mb_ucfirst(i18nDec("colUrgency")), "name"=>"urgency", "action"=>"field", "field"=>"urge"),
		array("txt"=>mb_ucfirst(i18nDec("colCriticality")), "name"=>"criticality", "action"=>"field", "field"=>"crit"),
		array("txt"=>mb_ucfirst(i18nDec("colPriority")), "name"=>"priority", "action"=>"field", "field"=>"prio")
	),
	array(
		array("txt"=>mb_ucfirst(i18nDec("colIdProject")), "name"=>"project", "action"=>"field", "field"=>"proj"),
		array("txt"=>mb_ucfirst(i18nDec("colIdActivity")), "name"=>"activity", "action"=>"field", "field"=>"act")
	),
	array(
		array("txt"=>mb_ucfirst(i18nDec("colEstimatedWork")), "name"=>"estimatedWork", "action"=>"field", "field"=>"work"),
		array("txt"=>mb_ucfirst(i18nDec("colIdContext")), "name"=>"context", "action"=>"field", "field"=>"ctxt1")
	),
	array(
		array("txt"=>mb_ucfirst(i18nDec("fileAttachment")), "name"=>"attachments", "action"=>"attachments")
	),
	array(
		array("txt"=>mb_ucfirst(i18nDec("paramTelegramBotCmdCreate")), "action"=>"confirm")
	)
);
...
class CommandHandler {
		...
		if (isset($this->input["action"])) {
			switch ($this->input["action"]) {
				case $commands["create"]:
					$userCanCreate = SqlElement::getSingleSqlElementFromCriteria("HabilitationOther", array(
						"idProfile" => $user->idProfile,
						"scope" => "tgBotCreate"
					));
					if ($userCanCreate->rightAccess == "1") {
						$this->startCreate();
					} else {
						$this->replyMessage->text = i18nDec("telegramBotMsgNoRightCreateCmd");
					}
					break;
				...
				case $commands["search"]:
					$userCanSearch = SqlElement::getSingleSqlElementFromCriteria("HabilitationOther", array(
						"idProfile" => $user->idProfile,
						"scope" => "tgBotSearch"
					));
					if ($userCanSearch->rightAccess == "1") {
						$this->startSearch();
					} else {
						$this->replyMessage->text = i18nDec("telegramBotMsgNoRightSearchCmd");
					}
					break;
	...
	private function startCreate($force=false) {
		...
		if ($force or $this->checkNotStarted()) {
			...
			foreach ($creatableClasses as $class => $attributes) {
	...
	private function startSearch($force=false) {
		...
		if ($force or $this->checkNotStarted()) {
			...
			$this->startElementChoice(getDisplayableClasses());
		}
	}

	/**
	 * Starts the process of choosing an element (class, project, element)
	 *
	 * @param $classes: array of selectable classes
	 * @return void
	 **/
	private function startElementChoice($classes) {
		$this->botUser->state = 100;
		$search = !$this->botUser->getData("choosingElement");

		if ($search) {
			$this->replyMessage->text = i18nDec("telegramBotMsgSearchClass");
		} else {
			$this->replyMessage->text = i18nDec("telegramBotMsgRefClass");
		}
		...
		if (count($classes) == 0) {
			if ($search) {
				$this->replyMessage->text = i18nDec("telegramBotMsgNoDisplayTemplate");
			} else {
				$this->replyMessage->text = i18nDec("telegramBotMsgNoClasses");
			}
			$this->endCommand();

		} else {
			foreach ($classes as $class) {
				$keyboard[] = array("text"=>i18nDec($class),"callback_data"=>"{'action':'chooseClass','class':'$class'}");
			}
		}
	...
	private function handleCallback() {
		...
		switch ($this->botUser->state) {
			...
			case 20:
				if ($cbAction === "field") {
					$this->chooseField($callback["field"]);
			...
			case 21:
				if ($cbAction == "none") {
					...
					if ($params[0] === "list") {
						...
						log_msg("Trying to leave project empty in path mode", TG_LOG_MSG);
					}
					...
					if ($path === null or count($path) == 0) {
						if ($this->botUser->getData("currentCommand") === "search") {
							...
							break;
						} else if ($this->botUser->getData("choosingElement")) {
							$this->botUser->setData("refElement", false);
							$this->fieldChoice();
							break;
						}
						$path = array();
			...
			case 40:
				...
				if (substr($cbAction, 0, 3) === "sel") {
					...
					if ($currentField == "context1") {
						...
						if ($currentField == "model" and !(isset($extra) and $extra == "unset")) {
			...
			//(Search) Class choice -> CHANGE V6.1.0
			case 100:
				//if ($cbAction == "chooseSearch") {
				if ($cbAction == "chooseClass") {
					$this->botUser->setData("searchClass", $callback["class"]);

					$startFromLast = $this->botUser->getData("startFromLast") ?? false;

					$this->projectChoice("path", true, true, 2, $startFromLast, true, $callback["class"]);
				}
			...
			case 120:
				if ($cbAction == "return") {
					...
					} else if ($currentCommand === "create" and $this->botUser->getData("choosingElement")) {

						$this->botUser->setData("refElement", true);
						$this->botUser->setData("refType", $class);
						$this->botUser->setData("refId", $id);

						$data = $this->botUser->getData();
						unset($data["choosingElement"]);
						$this->botUser->setData(null, $data);
	...
	private function fieldChoice() {
		...
		$requiredFields = requiredFields(getEditableFields($createClass));
		...
		foreach ($requiredFields as $key => $value) {
			if (!isset($data[$key]) or !$data[$key]) {
				...
				if (isset($value["combine"])) { //if attribute "combine", check that all combined fields are set, if so, then this one is set
					$allSet = true;
					foreach ($value["combine"] as $combinedField) {
						if (!isset($data[$combinedField]) || !$data[$combinedField]) { //if combined field is not set
							$allSet = false;
						}
					}

					if ($allSet) {
						$this->botUser->setData($key, true);
						continue;
					}
				}

				$this->chooseField($key);
				return;
			}
		}
	...
	/**
	 * Finalizes choice of field (called directly for required fields
	 * or when a button is pressed from the fields menu)
	 *
	 * @param $field: name of the chosen field
	 * @return void
	 **/
	private function chooseField($field) {
		$field = longField($field);

		$fieldInfo = array();

		$fields = getEditableFields($this->botUser->getData("createClass"));
		if (array_key_exists($field, $fields)) {
			$fieldInfo = $fields[$field];
		
		} else if (array_key_exists($field, array_column($fields, null, "name"))) {
			$fieldInfo = array_column($fields, null, "name")[$field];
		}

		switch ($field) {
			case "name":
				$this->askField("name","name","text", null, null, null, !isset($fieldInfo["required"]));
				break;

			case "description":
				$this->askField("desc","description","text", null, null, null, !isset($fieldInfo["required"]));
				break;

			case "model":
				$this->botUser->setData("currentField","model");
				$this->askField("model","model","classTicketTemplate", null, array("idle" => "0"), null, !isset($fieldInfo["required"]));
				break;
			...
			case "responsible":
				...
				if ($this->botUser->getData("project")) {
					...
					foreach ($rows as $row) {
						$resps[] = array("id"=>$row["id"],"text"=>$row["name"]);
					}
					...
					foreach ($results as $resp) {
						$resps[] = array("id"=>$resp->id,"text"=>$resp->name);
					}
				}

				$this->askField("resp","responsible","list",null,null,$resps, !isset($fieldInfo["required"]));

				break;

			case "project":
				$params = $fieldInfo["params"] ?? array("list", false);

				$this->projectChoice(...$params);

				if (!isset($fieldInfo["required"])) {
					$this->replyMessage->markup["inline_keyboard"][] = array(array(
						"text" => i18nDec("telegramBotUnsetField"),
						"callback_data" => "{'action':'none'}"
					));
				}
			...
			case "activity":
				...
				$this->askField("act","activity","classActivity",null,$crits, null, !isset($fieldInfo["required"]));
				break;
			...
			case "context1":
				...
				$this->askField(shortField($field), $field, "class".ucfirst($field), null, $crits, null, !isset($fieldInfo["required"]));

				break;

			case "estimatedWork":
				$this->askField("work", "estimatedWork", "text", null, null, null, !isset($fieldInfo["required"]));
				break;
			
			case "refElement":
				$this->botUser->setData("choosingElement",true);

				if (isset($fieldInfo["startFromLastPath"])) {
					$this->botUser->setData("startFromLast", true);
				}

				$this->startElementChoice($fieldInfo["classes"] ?? array());
	...
	private function askField($short,$field,$type="text",$validateCb=null,$crits=array("idle" => "0"),$list=array(),$optional=true) {
	...
	private function projectChoice($mode="path", $canReturn=false, $canChooseSub=false, $mustHaveElements=0, $startFromLast=false, $skip=true, $class=null) {
		...
		$this->botUser->setData("currentField", "project");
		...
		if ($mode === "path") {
			...
			$projects = getProjectsFromPath($path, $this->botUser->idUser, $mustHaveElements, $class);
	...
	private function chooseProject($chooseSub=false) {
		...
		if ($currentCommand === "search" || ($currentCommand === "create" && $this->botUser->getData("choosingElement"))) {
			$this->elementChoice();
	...
	private function elementChoice() {
		...
		foreach ($elements as $element) {
			...
			$buttons[] = array("text"=>$element->name.$emojis, "callback_data"=>"{'action':'chooseElement','id':'$id'}");
		}
	...
	private function displayElement($class, $id, $canReturn=true) {
		...
		$element = new $class($id);
		$user = new User($this->botUser->idUser);
		$accessRight = securityGetAccessRightYesNo($element->getMenuClass(), "read", $element, $user);

		if ($accessRight === "NO") {
			$this->replyMessage->text = i18nDec("telegramBotMsgNoRightDisplay");
			return;
		}
		...
		if ($template and $template->template) {
			...
			$result = i18nDec("telegramBotMsgNoDisplayTemplateElmt");
		}
	...
	private function getDisplayTemplate($element) {
		...
		$idMailable = SqlList::getIdFromName("Mailable", $class, true); //true -> disable cache
	...
	private function getFieldsMenu($class) {
		...
		$displayableFields = fieldsToDisplay(getEditableFields($class));
		...
		foreach ($createOptions as $line) {
			...
			foreach ($line as $button) {
				...
				if (isset($cbData["name"])) {
					...
					if (array_key_exists($cbData["name"], $displayableFields)) {
						$fieldInfo = $displayableFields[$cbData["name"]];
					
					} else if (array_key_exists($cbData["name"], array_column($displayableFields, null, "name"))) {
						$fieldInfo = array_column($displayableFields, null, "name")[$cbData["name"]];
					}
					if ($fieldInfo === null) {
						continue; //Don't show the button
					}
					...
					if ($cbData["name"] == "context") {
						...
						$value = $this->botUser->getData($cbData["name"]);
						$txt .= " $setEmoji";

						if (in_array($cbData["name"], array("urgency","criticality","priority"))) {
							$class = ucfirst($cbData["name"]);
							$elmt = new $class($value);

							if (property_exists($elmt, "tgEmoji")) {
								$emoji = $elmt->tgEmoji;
								$txt .= " ($emoji)";
							}
						} else if ($cbData["name"] === "responsible") {
	...
	private function createElement() {
		...
		$user = new User($this->botUser->idUser);
		$accessRight = securityGetAccessRightYesNo($element->getMenuClass(), "create", $element, $user);

		if ($accessRight === "NO") {
			$this->replyMessage->text = i18nDec("telegramBotMsgNoRightCreate");
			return;
		}

		$fieldsAttributes = getEditableFields($class);

		if (isset($creatableClasses[$class]["persistent"]) and $creatableClasses[$class]["persistent"]) {

			$persistent = $this->botUser->getData("persistent") ?? array();
			$classes = $persistent["fields"] ?? array();
			$fields = $classes[$class] ?? array();
			$byId = array_column($fieldsAttributes, null, "id");

			foreach ($elementData as $field => $value) {
				if (array_key_exists($field, $byId)) {
					if (isset($byId[$field]["persistent"]) and $byId[$field]["persistent"]) {
						$fields[$field] = $value;
					}
				}
			}
		...
		$msg = $elementData["reference"] ? i18nDec("telegramBotMsgCreated") : i18nDec("telegramBotMsgCreatedNoRef");
	...
	private function getCreateData() {
		...
		foreach ($data as $field => $value) {
			...
			if (array_key_exists($field, $availableFields)) {
				$fieldInfo = $availableFields[$field];
			
			} else if (array_key_exists($field, array_column($availableFields, null, "name"))) {
				$fieldInfo = array_column($availableFields, null, "name")[$field];
			}
			if ($fieldInfo !== null) {
				if (!isset($fieldInfo["notProperty"])) {
					$elementData[$fieldInfo["id"]] = $value;
				}
			}
		}
	...
	/**
	 * Fetches persistent data for the class
	 * For combined fields, the parent field
	 * is set if all children are set
	 *
	 * @return void
	 **/
	private function getPersistentFields() {
		global $creatableClasses;

		$class = $this->botUser->getData("createClass");
		
		if (isset($creatableClasses[$class]["persistent"]) and $creatableClasses[$class]["persistent"]) {
			$fields = getEditableFields($class);
			...
			if (count($values) > 0) {
				foreach ($fields as $field => $attributes) {
					...
					if (isset($attributes["persistent"]) and $attributes["persistent"]) {

						if (isset($attributes["id"])) {

							if (isset($values[$attributes["id"]])) {
								$this->botUser->setData($name, $values[$attributes["id"]]);
							}
						}
					}
				}
...
function getEditableFields($class) {
	$fields = array(
		"Ticket" => array(
			"name" => array("id"=>"name","required"=>true),
			"description" => array("id"=>"description"),
			"type" => array("id"=>"idTicketType"),
			"project" => array("id"=>"idProject",
				"params"=> array("list", false)),
			"responsible" => array("id"=>"idResource"),
			"urgency" => array("id"=>"idUrgency"),
			"criticality" => array("id"=>"idCriticality"),
			"context1" => array("id"=>"idContext1", "name" => "context"),
			"context3" => array("id"=>"idContext3", "hidden"=>true),
			"activity" => array("id"=>"idActivity"),
			"estimatedWork" => array("id"=>"estimatedWork"),
			"model" => array("notProperty"=>true),
			"attachments" => array("notProperty"=>true)
		),
		"SubTask" => array(
			"refElement" => array("id"=>"refElement","notProperty"=>true,"required"=>true,
				"type"=>"element", "classes"=>array("Activity","Ticket","Action"), "startFromLastPath"=>true,"combine"=>array("refType","refId")),
			"refType" => array("id"=>"refType","hidden"=>true,"persistent"=>true),
			"refId" => array("id"=>"refId","hidden"=>true,"persistent"=>true),
			"name" => array("id"=>"name","required"=>true),
			"priority" => array("id"=>"idPriority","persistent"=>true),
			"responsible" => array("id"=>"idResource","persistent"=>true)
		),
		"Question" => array(
			"name" => array("id"=>"name","required"=>true),
			"project" => array("id"=>"idProject","required"=>true,
				"params"=> array("path", false, false, 0, true, false)),
			"description" => array("id"=>"description","required"=>true),
			"responsible" => array("id"=>"idResource","required"=>true),
			"type" => array("id"=>"idQuestionType", "hidden"=>true)
		)
	);
...
/**
 * Filters a list of fields (result of getEditableFields) and returns only those to diplay (not required and not "hidden")
 *
 * @return array of fields (see getEditableFields)
 **/
function fieldsToDisplay($fields) {
	$result = array_filter($fields, function($v) {
		//return (!isset($v["required"])) && (!isset($v["hidden"]));
		return !isset($v["hidden"]);
	});
...
/**
 * Filters a list of fields (result of getEditableFields) and returns only required ones
 *
 * @return array of fields (see getEditableFields)
 **/
function requiredFields($fields) {
	$result = array_filter($fields, function($v) {
		return isset($v["required"]);
	});
...
/**
 * Returns the long field name from the short
 *
 * @param $field: short name
 * @return long name
 **/
function longField($field) {
	global $FIELDS;
	return $FIELDS[$field] ?? $field;
}
/**
 * Returns the short field name from the long
 *
 * @param $field: long name
 * @return short name
 **/
function shortField($field) {
	global $FIELDS;
	return array_flip($FIELDS)[$field] ?? $field;
}

/**
 * Capitalizes a string but keeps accents (élément -> Élément)
 *
