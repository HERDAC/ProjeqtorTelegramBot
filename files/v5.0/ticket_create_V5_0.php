...
$msgs = array(
	...
	"about" => "*Bot Telegram pour ProjeQtOr (HERDAC)*".
			 "\n".
			 "\n__*$VERSION*__ ([Changelog](https://github.com/LordBaryhobal/ProjeqtorTelegramBot/blob/main/CHANGELOG.md))".
			 "\n".
			 "\nCréateur:".
			 "\n        @Baryhobal".
	...
	"multiple-reference" => "Plusieurs éléments ont la référence :reference.\nSéléctionnez-en un",
	...
	"chooseNote" => "Choisissez une note ou créez-en une",
	"newNote" => "Écrivez votre note",
	"newNoteReply" => "Réponse à la note #:id\n```\n:content```\n\nÉcrivez votre note"
);
...
$states = array(
	...
	300 => "Reference",
	310 => "Reference: multiple elements",
	400 => "Attachments"
);
...
function displayElement($id, $class, $customButtons=false) {
	...
	if (!$customButtons) {
		...
		$buttons[] = array("text"=>"Notes","callback_data"=>"{'action':'notes'}");
		...
		$msg = $result;
		$markup = array("inline_keyboard"=>array($buttons, array(array("text"=>"Retour","callback_data"=>"{'action':'return'}"))));
...
function getDisplayElementFromReference($reference, &$data, &$state) {
	global $msg, $msgs;

	$element = getElementFromReference($reference);
	...
	if (count($element) == 0) {
		...
		chooseReferenceElement($element, $data, $state);
		$msg = str_replace(":reference", $reference, $msgs["multiple-reference"]);
	}
}

function chooseReferenceElement($elements, &$data, &$state) {
	global $markup;
	...
	foreach ($elements as $element) {
		...
		$buttons[] = array("text"=>$name, "callback_data"=>"{'action':'refElmt','class':'$class','id':$id}");
	}
...
function displayNotes($element) {
	...
	$buttons[] = array(
		array("text"=>"Retour", "callback_data"=>"{'action':'return'}"),
		array("text"=>"Créer une note", "callback_data"=>"{'action':'newNote'}")
	);
	$markup["inline_keyboard"] = $buttons;
}

function displayNote() {
	global $msg, $data, $markup;

	$note = new Note($data["noteId"]);
	$id = $note->id;
	...
	$markup["inline_keyboard"] = array(
		array(
			array("text"=>"Retour", "callback_data"=>"{'action':'return'}"),
			array("text"=>"Répondre", "callback_data"=>"{'action':'newNote'}")
		)
	);
}

function newNote() {
	global $msg, $msgs, $state;
	$msg = $msgs["newNote"];
	$state = 160;

	if (isset($callback["id"])) {
		$note = new Note($callback["id"]);
		$msg = str_replace(
			array(":id",":content"),
	...
	switch ($action) {
				...
				switch ($state) {
					...
					case 160: {
						...
						if (isset($data["noteId"])) {
							$note->idNote = $data["noteId"];
							$parent = new Note($data["noteId"]);
					...
					case 300: {
						getDisplayElementFromReference($input["content"], $data, $state);
				...
				switch ($state) {
					...
					case 140: {
						if ($cbAction == "return") {
							$state = 130;
							$path = $data["displayPath"];
							displayElement(array_pop($path), $data["displayPath"][0]);
						
						} else if ($cbAction == "selNote") {
							$state = 150;

							$data["noteId"] = $callback["id"];
							displayNote();
		...
		case $commands["reference"]: {
			if ($state == 0) {
				...
				if (count($args) == 2) {
					getDisplayElementFromReference($args[1], $data, $state);
...
if ($msg != "") {
	...
	if (!$modify) {
		...
		$userVars["choose_msg_id"] = $input["messageId"];
		$result = send_msg($replyMsg, "editMessageText");
	}
}
