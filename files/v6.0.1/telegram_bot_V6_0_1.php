...
class CommandHandler {
	...
	private function askField($short,$field,$type="text",$validateCb=null,$crits=array("idle" => "0"),$list=array(),$optional=true) {
		...
		//$this->botUser->setData("validateCb",$validateCb);
	...
	private function getDisplayTemplate($element) {
		...
		error_log($class);
		error_log($typeName);
		error_log($idMailable);
		error_log(json_encode($withStatusWithType));
		error_log(json_encode($withoutStatusWithType));
		error_log(json_encode($withStatusWithoutType));
		error_log(json_encode($withoutStatusWithoutType));
		*/
